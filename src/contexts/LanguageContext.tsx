...
const translations: Record<string, Record<"ru" | "en", string>> = {
  ...
  "create.htmlCodeHint": { ru: 'Вставьте полный HTML-код баннера с тегом <img src="http://..."> или <img src="https://...">. Отдельно загружать изображение не нужно. Размер содержимого должен быть {w}×{h}px.', en: 'Paste the full banner HTML with an <img src="http://..."> or <img src="https://..."> tag. No separate image upload is needed. The content size must be {w}×{h}px.' },
  "create.htmlImageUrlRequired": { ru: "Добавьте тег <img> с корректной абсолютной ссылкой на изображение (HTTP/HTTPS).", en: "Add an <img> tag with a valid absolute image URL (HTTP/HTTPS)." },
