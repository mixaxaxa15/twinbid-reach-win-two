...
function NetworkStage() {
  const formats = [
    { name: "Popunder", inset: "12%", duration: "34s", delay: "-2s" },
    { name: "Native", inset: "12%", duration: "34s", delay: "-11s" },
    { name: "Banner", inset: "25%", duration: "25s", delay: "-5s" },
    { name: "In-page Push", inset: "25%", duration: "25s", delay: "-17s" },
  ];
  ...
  return (
    <motion.div
      ...
      aria-hidden
    >
      <div className="absolute inset-0 landing-global-grid opacity-50" />
      ...
      <div className="absolute left-1/2 top-1/2 z-10 flex h-32 w-32 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-primary/35 bg-background/90 shadow-[0_0_90px_hsl(var(--primary)/0.26)] backdrop-blur-xl md:h-40 md:w-40">
        <span className="font-display text-xl font-medium tracking-[-0.04em] text-foreground md:text-2xl">TwinBid</span>
      </div>
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-[1400px] items-center gap-12 px-5 md:px-8 lg:grid-cols-[1.04fr_0.96fr] lg:gap-14">
        <div className="max-w-[760px]">
          ...
          >
            ...
            <span className="landing-outline-text block">{title2}</span>
          </motion.h1>
          ...
          >
            {t("hero.subtitle")} <span className="text-foreground">{t("hero.subtitleSites")}</span> {t("hero.subtitleEnd")}
          </motion.p>
          ...
          >
            ...
            />
            <a href="#benefits" className="landing-button landing-button-ghost justify-center px-7 py-4 text-[14px]">
              {t("hero.learnMore")} <ArrowDownRight className="h-4 w-4" />
            </a>
          </motion.div>
        </div>
